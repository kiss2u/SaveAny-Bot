...
在 [Release](https://github.com/kiss2u/SaveAny-Bot/releases) 页面下载对应平台的二进制文件.
...
创建文件 <code>/etc/init.d/saveanybot</code> ，参考 <a href="https://github.com/kiss2u/SaveAny-Bot/blob/main/docs/confs/wrt_init" target="_blank">wrt_init</a> 并自行修改:
...
创建文件 <code>/usr/bin/sabot</code> ，参考 <a href="https://github.com/kiss2u/SaveAny-Bot/blob/main/docs/confs/wrt_bin" target="_blank">wrt_bin</a>  并自行修改，注意此处文件编码仅支持 ANSI 936 .
...
下载 [docker-compose.yml](https://github.com/kiss2u/SaveAny-Bot/blob/main/docker-compose.yml) 文件, 在同目录下新建 `config.toml` 文件, 参考 [config.example.toml](https://github.com/kiss2u/SaveAny-Bot/blob/main/config.example.toml) 编辑配置文件.
